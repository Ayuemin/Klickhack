...
# Snap destinations to the nearest road node that is actually reachable from PSCH-46.
# This avoids falsely declaring a settlement unreachable because its nearest tiny service/track component is isolated.
code=code.replace(
...
"r,km=route_to((float(s['lat']),float(s['lon'])),0.35 if s.get('isOfficial') else None)"
)
code=code.replace(
...
"s['route']=r; s['distanceKm']=km; s['via']=route_via(r,official,s.get('id'),km or 0); s['routeVerified']=True"
)
code=code.replace(
"if failures:\n    print('UNROUTABLE:',failures[:30]); raise RuntimeError(f'{len(failures)} training points have no safe road route')",
"if failures:\n    print('UNROUTABLE:',failures[:30])\n    official_failures=[name for name,is_official in failures if is_official]\n    if official_failures: raise RuntimeError(f'{len(official_failures)} official settlements have no safe reachable road within 350 m: {official_failures}')"
)
